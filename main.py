...
class KameraPaneli(FloatLayout):
    ...
    def _kurulum(self):
        # ── En guvenli yontem: ──────────────────────────────────────────────────
        # 1) Camera widget GIZLI (size 0,0) — sadece texture uretir
        # 2) KivyImage widget tam ekran gosterir
        # 3) Clock ile her 33ms (30fps) Image.texture guncellenir
        # Hicbir rotation, numpy, canvas manipulation yok → cokme olmaz

        # Gizli Camera — sadece texture uretmek icin
        self._kamera = Camera(
            index=0,
            resolution=(1280, 720),
            size=(1, 1),
        ...
        # Gorsel Image widget — tam ekran, Android icin 90 derece donuk
        W = Window.width
        H = Window.height

        if IS_ANDROID:
            # Kamera texture yatay geliyor (W>H).
            # Image'i H genislik, W yukseklikte yap, -90 ile don -> W x H dikey
            img_w = H
            img_h = W
            img_x = -(img_w - W) / 2   # yatayda ortala
            img_y = -(img_h - H) / 2   # dikeyde ortala
            rot   = 180
        else:
            img_w, img_h = W, H
            img_x, img_y = 0, 0
        ...
        # Her 33ms texture'i Image'a aktar (30 fps)
        def _guncelle(dt):
            tex = self._kamera.texture
            if tex:
                self._img.texture = tex
        Clock.schedule_interval(_guncelle, 1.0 / 30.0)
        self._clock_ev = _guncelle

        # UI katmani: scatter'in USTUNE degil, ana FloatLayout'a ekleniyor
        ui = BoxLayout(
